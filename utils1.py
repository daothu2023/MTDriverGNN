...
def build_dataset_directly(ppi_cpdb_path, features_path, driver_labels_path, telomere_labels_path=None):
    """
    Build a PyG Data object directly from user-supplied file paths.
    Used by --select_model when the user provides their own cancer data
    (i.e. a cancer type not in the 12 built-in types).
    ...
    driver_labels_path   : path to user label CSV (columns: Gene, Labels)
    ...
    labeled_idx  : tensor of indices where driver label != -1
    """
    node_names, node_to_idx, edge_index, num_nodes = load_graph_from_ppi_cpdb(ppi_cpdb_path)
    x = build_node_features(features_path, node_names, node_to_idx, num_nodes, edge_index)
...
def model_selection(
    ...
    *_options          : hyperparameter grid (tuples/lists)
    ...
    # ── Step 9: save model + test_indices ─────────────────────────────────────
