...
def build_cross_disease_pretrain_labels(node_to_idx, target_cancer, driver_labels, cancer_data_paths):
    ...
    num_excluded = int((target_labeled_mask & ((pretrain_labels == 1) | (pretrain_labels == 0))).sum().item())
    print(f"[Cross-disease pretrain labels] pos={num_pos}, neg={num_neg}, excluded (target-labeled)={num_excluded}")
