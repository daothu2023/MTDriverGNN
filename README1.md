...
For **Step 2**, only `--features` is needed. Labels and test indices are
loaded automatically from the saved checkpoint.
...
# Step 2: predict (only --features needed, no --labels)
...
| `--labels` | `None` | `--select_model` (new cancer type only) | Path to driver label CSV. Not needed for `--predict` |
