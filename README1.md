...
configuration and saved to `results/best_model_<CANCER>.pt`.

**Step 2 — Predict:** loads the saved model, computes AUPRC on the
held-out test set, and ranks all unlabeled genes (driver label unknown)
by their predicted driver score.
...
Prepare two CSV files following the format described in the
**Input Data Format** section below, then run:
...
```bash
# Keep only the top 100 highest-scoring genes
...
| `--features` | `None` | `--select_model`, `--predict` | Path to feature matrix CSV |
| `--labels` | `None` | `--select_model` | Path to driver label CSV |
...
| `--top_k` | `None` | `--predict` | Save only the top-K genes by driver score. Default: save all |
