...
def run_predict(args, selected_device):
    ...
    if args.features is not None:
        ...
        # Labels are not needed for --predict: test_indices are loaded
        # from the model checkpoint, and unlabeled genes are identified
        # from the checkpoint as well.
        data, node_to_idx, _ = build_dataset_directly(
            ppi_path, args.features,
            driver_labels_path=None,
    ...
    # ── Ranking: unlabeled genes only (label == -1) ────────────────────────
    labels_cpu = data.y.cpu().numpy()
    unlabeled_mask = (labels_cpu == -1)
    unlabeled_genes  = node_names[unlabeled_mask]
    unlabeled_scores = scores[unlabeled_mask]

    rank_order = np.argsort(unlabeled_scores)[::-1]
    results_df = pd.DataFrame({
        "gene":         unlabeled_genes[rank_order],
        "driver_score": unlabeled_scores[rank_order],
        "rank":         np.arange(1, len(rank_order) + 1),
    })

    if args.top_k is not None:
        results_df = results_df.head(args.top_k)
        print(f"[Predict] Keeping top {args.top_k} unlabeled genes.")

    os.makedirs(os.path.dirname(os.path.abspath(args.output)), exist_ok=True)
    results_df.to_csv(args.output, index=False)
    print(f"\n[Done] Ranking saved to: {args.output}")
    print(f"       Unlabeled genes ranked: {len(results_df)}")
